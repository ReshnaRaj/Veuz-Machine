...
const tickets = [
  {
    ...
    features: [
      "Access to Conference & Investor Lounge",
      "All Keynotes Events",
    ...
    >
      ...
      <div className={`p-3 ${ticket.color} bg-opacity-90`}>
        <div className="flex justify-between items-center">
          ...
          {ticket.badge && (
            <span className="text-xs bg-white text-green-600 px-2 py-1 rounded font-bold">
              {ticket.badge}
            </span>
          )}
        </div>
        <p className="text-xs text-green-200 underline cursor-pointer">
          {ticket.details}
        </p>
      </div>
      ...
      <div className="relative">
        <div className="absolute inset-0 backdrop-blur-sm bg-black/60 rounded-none"></div>
      ...
      <div className="p-3 flex justify-between items-center border-t border-gray-700 bg-black bg-opacity-70">
        ...
        <button className="bg-white text-black text-xs font-bold px-4 py-1 rounded" onClick={()=>window.location.href = "/register"}>
          BUY NOW
        </button>
      </div>
    </div>
